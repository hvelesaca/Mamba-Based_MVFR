...
from model_2 import SimpleFoulModel, SimpleActionModel
...
def evaluate_models(foul_model, action_model, test_loader, device="cuda" if torch.cuda.is_available() else "cpu"):
    foul_model.eval()
    action_model.eval()
    ...
    with torch.no_grad():
        for batch_clips, foul_labels, action_labels, action_ids in tqdm(test_loader, desc="Evaluating Test Set", unit="batch"):
            batch_clips = batch_clips.to(device, non_blocking=True)
            
            foul_logits = foul_model(batch_clips)
            ...
            action_logits = action_model(batch_clips)
    ...
    prediction_file = f"results/predictions_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    foul_model = SimpleFoulModel(agr_type='max').to(device)
    foul_model.load_state_dict(torch.load("models/best_foul_model.pth", map_location=device))
    
    action_model = SimpleActionModel(agr_type='max').to(device)
    action_model.load_state_dict(torch.load("models/best_action_model.pth", map_location=device))
    
    print("\nEvaluating Trained Models on Test Set...")
    evaluate_models(foul_model, action_model, test_loader, device)
