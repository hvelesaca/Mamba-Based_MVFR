...
def compute_performance(dataset, predictions):
    foul_true, foul_pred = [], []
    ...
    if not foul_true:
        ...
        foul_metrics = {"Accuracy": 0.0, "Balanced Accuracy": 0.0}
    else:
    ...
    if not action_true:
        ...
        action_metrics = {"Accuracy": 0.0, "Balanced Accuracy": 0.0}
    else:
...
def predict_test_split(model, test_loader, device="cuda" if torch.cuda.is_available() else "cpu"):
    ...
    metrics = compute_performance(test_loader.dataset, test_predictions)
    return metrics
...
if __name__ == "__main__":
    ...
    # Load model weights
    model_path = "models/best_multitask_mamba_model.pth"
